...
from ..model.function import Function, FunctionType
from .store import StoreGraphicsItem
from .mapping import MappingGraphicsItem, ControlMappingGraphicsItem
from .colors import STD_COLOR, TRUE_COLOR, FALSE_COLOR, FLOW_COLOR
...
class FunctionGraphicsItem(GraphicsObject):
    ...
    def __init__(self, function: Function, view, test: bool = None):
        ...
        self.setColor(STD_COLOR if function.type == FunctionType.LINEAR else FLOW_COLOR)
        if test is not None:
            if function.type == FunctionType.LINEAR:
                self.setColor(TRUE_COLOR if test else FALSE_COLOR)
            else:
                self.testPen = QPen(TRUE_COLOR if test else FALSE_COLOR, 2)
                self.testPen.setStyle(Qt.PenStyle.CustomDashLine)
                self.testPen.setDashPattern([4, 8])  # 4px dash, 8px gap
    ...
    def setColor(self, color: QColor):
        ...
        # Base state
        self.pen = QPen(pen_color, 2)

        # Hover state -> thicker border, still transparent fill
        self.hoverPen = QPen(pen_color, 4)

        # Selected state -> thicker border + semi-transparent fill
        self.selectPen = QPen(pen_color, 4)

        # Brushes
        self.brush = QBrush(Qt.BrushStyle.NoBrush)  # normal: fully transparent
        self.hoverBrush = QBrush(Qt.BrushStyle.NoBrush)  # hover: still transparent

        select_color = color.lighter(115)
        select_color.setAlpha(40)  # very transparent hue
    ...
    def paint(self, p, *args):
        ...
        # Draw dashed test border if applicable
        if self.testPen:
            p.setPen(self.testPen)
    ...
    def elk(self):
        elk = {
            ...
            "layoutOptions": {
                ...
                "elk.padding": (
                    f"[top=10.0,left=20.0,bottom=10.0,right=20.0]"
                    if self.function.internal
                    else f"[top=0.0,left=0.0,bottom=0.0,right=0.0]"
                ),
            },
            "labels": [
                {
