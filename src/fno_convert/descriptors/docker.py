...
from ..util.mapping import ValueMapping, MappingNode
...
class DockerfileDescriptor:
    ...
    def describe_file(self, path):
        ...
        if not self.g.exists(file_uri):
            ...
            FnOBuilder.describe_composition(
                self.g, comp_uri, self.mappings, represents=fun_uri
            )
    ...
    def handle_mapping(self, mapfrom, mapto):
        self.mappings.append(ValueMapping(mapfrom, mapto))
    ...
    def handle_entrypoint(self, values):
        ...
        entrypoint_cmd = MappingNode().set_function_par(
            call_uri, Prefix.do()["entrypointInputCommand"]
        )
        self.handle_mapping(cmd, entrypoint_cmd)

        # Set entrypoint command parameters
        if len(values) > 1:
            entrypoint_cmd_params = MappingNode().set_function_par(
                call_uri, Prefix.do()["entrypointInputParamList"]
            )
            values = MappingNode().set_constant(
                PythonMapper.value_to_term(self.g, values[1:])
            )
    ...
    def handle_cmd(self, values):
        ...
        if len(values) > 1:
            cmd_params = MappingNode().set_function_par(
                call_uri, Prefix.do()["cmdInputParamList"]
            )
            values = MappingNode().set_constant(
                PythonMapper.value_to_term(self.g, values[1:])
            )
    ...
    def handle_run(self, value):
        ...
        run_cmd = MappingNode().set_function_par(
            call_uri, Prefix.do()["runInputCommand"]
        )
    ...
    def handle_workdir(self, value):
        ...
        dir_input = MappingNode().set_function_par(
            call_uri, Prefix.do()["workdirInput"]
        )
