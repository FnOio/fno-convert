    ...
    def provenance(self, fun: Function, *args, **kwargs):
        ...
        for i, param in enumerate(params):
            ...
            FnOBuilder.describe_parameter(
                self.pg, uri, Prefix.ns("xsd").string, self.pg.get_predicate(param)
            )
            param_uris[param] = uri

            mapping = ValueMapping(
                MappingNode().set_function_out(self.fun_uri, uri),
                MappingNode().set_function_out(fun_uri, param),
            )
            FnOBuilder.add_value_mapping(self.pg, comp_uri, mapping)
        ...
        FnOBuilder.describe_output(
            self.pg, uri, self.pg.get_output_type(output), self.pg.get_predicate(output)
        )
        output_uris.append(uri)

        mapping = ValueMapping(
            MappingNode().set_function_out(fun_uri, output),
            MappingNode().set_function_out(self.fun_uri, uri),
        )
        FnOBuilder.add_value_mapping(self.pg, comp_uri, mapping)
        ...
        if self.pg.has_self_output(fun_uri):
            ...
            FnOBuilder.describe_output(
                self.pg,
                uri,
                self.pg.get_output_type(output),
                self.pg.get_predicate(output),
            )
            output_uris.append(uri)
        ...
        FnOBuilder.describe_function(
            self.pg, self.fun_uri, self.tag, param_uris.values(), output_uris
        )
        ...
        for map_uri in self.pg.get_mapping(fun_uri):
            ...
            FnOBuilder.describe_mapping(
                ...
                lists,
                dicts,
                context_output=output_uris[1] if len(output_uris) > 1 else None,
            )
