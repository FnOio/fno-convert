...
class Terminal(ValueStore):
    ...
    def __init__(self, fun, uri: URIRef, pred: URIRef, type=None, is_output=False, param_mapping: ParameterMapping=None) -> None:
        try:
    ...
    def get(self, strat=None, key=None):
        ...
        if (self.param_mapping.is_type(MappingType.POSITIONAL) and self.strat == "toList") or \
            (self.param_mapping.is_type(MappingType.POSITIONAL) and self.param_mapping.index) or \
            self.param_mapping.is_type(MappingType.LIST):
                return self.to_list()
